...
/**
 * Helper to print a passing test result.
 *
 * @param string $name Test name.
 */
function bugsneak_test_pass( $name ) {
	echo "  ✅  PASS  │  {$name}\n"; }
/**
 * Helper to print a failing test result.
 *
 * @param string $name   Test name.
 * @param string $detail Optional failure detail.
 */
function bugsneak_test_fail( $name, $detail = '' ) {
	echo "  ❌  FAIL  │  {$name}  │  {$detail}\n"; }
// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped

/**
 * Count rows in the log table.
 *
 * @return int
 */
...
Settings::save( array( 'max_rows' => 10 ) );
...
$bugsneak_test_rows = bugsneak_test_count_rows();
// Restore original.
Settings::save( array( 'max_rows' => $bugsneak_test_original_settings['max_rows'] ) );

if ( 10 === $bugsneak_test_rows ) {
...
// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- CLI test utility
...
if ( isset( $bugsneak_test_snippet['truncated'] ) && true === $bugsneak_test_snippet['truncated'] ) {
...
$bugsneak_test_required = array( 'error_hash', 'last_seen', 'created_at', 'error_type', 'share_token' );
